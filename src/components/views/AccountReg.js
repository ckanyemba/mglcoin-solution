...
function AccountReg(props) {
  ...
  const verifyInvite = (code) => {
      return axios
        .post(serverUrl + "users/inviteverify", {
          email: email,
          inviteCode: code,
          locale: localStorage.getItem("locale") || "Mn"
        })
        .then((response) => {
          if (response.data.response) {
            openNotification(
              t("Success"),
              t("Invite code sent successfully"),
              true,
              false
            );
            setMessage({
              style: "text-green-500",
              val: 1,
              data: t("Invite code sent successfully!")
            });
          } else {
            openNotification(
              t("Fail!"),
              t("Invite code not sent!"),
              false,
              false
            );
            setMessage({
  ...
  // Function to handle sending invite code email
  ...
  //Problem here. hasErros is using a function instead const
  const hasErrors = (fieldsError) => {
    const emailErrors = fieldsError["email"];
    const passwordErrors = fieldsError["password"];
    const confirmPasswordErrors = fieldsError["confirm_password"];

    if (emailErrors || passwordErrors || confirmPasswordErrors) {
      return true; // There are validation errors in email, password, or confirmPassword
    }
        ...
        <Form.Item name="inviteCode" rules={[{ max: 10 }]}>
          <Input
            ...
            suffix={
              <>
                <a
                  onClick={handleSendInviteCodeEmail}
                  className="bg-gray-800 text-md text-white rounded-xl py-1 px-4"
                >
                  {t("Receive")}
                </a>
